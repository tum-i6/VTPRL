...
There is no inverse kinematics calculation supported here (see iiwa_sample_env for IK), only joint velocity control
is supported with it. Unity is used as the main simulator for physics/rendering computations. The Unity interface
receives joint velocities as commands and returns joint positions and velocities. The class presents a way to alternate
between numeric observations and image observations, how to parse images returned from the Unity simulator and how to
reset the environments in the simulator
"""
...
class IiwaJointVelEnv(core.Env):
    ...
    def __init__(self, id, max_ts, config):
        ...
        if self.use_images:
            # if using images make 100x100 pixels grayscale observation space
    ...
    def _get_reward(self):
        ...
        return - 1 * (absolute_distance + self.collided)
    ...
    def _convert_observation(self, new_observation):
        ...
        in rad/sec, x, y, and z for ee, t and o are the coordinates and and rx, ry, and rz for ee, t and o are the
        quaternion x, y and z components of the rotation for the end-effector, the target and the object(box)
        ...
        self.collided = self.simulator_observation[-1]
    ...
    def _get_terminal(self):
        ...
        """
        dist = np.sqrt(np.power(self.target_x - self.ee_x, 2) + np.power(self.target_y - self.ee_y, 2)
                       + np.power(self.target_z - self.ee_z, 2))
        # if dist < 0.3:
        #    return True
    ...
    def _retrieve_image(self, observation):
        ...
        # return np.zeros(shape=(100, 100, 1),
        #        dtype=np.uint8)
