...
def get_env(agent, root, sim, gym_cfg, manip_env, observation, reward_dict, log_dir):
    ...
    def create_env(id=0):
        ...
        # model-based control with P-controller available
            ...
            env = WarehouseUnityEnv(max_time_steps=gym_cfg['max_time_step'], env_id=id, config=gym_cfg['warehouse_gym_environment'])
    ...
    env = SimulatorVecEnv(env, agent, root, sim, gym_cfg, manip_env, reward_dict=reward_dict)   # Set vectorized env
...
if __name__ == "__main__":
    ...
    # Build env #
    env = get_env(agent, root, sim, gym, manip_env, observation, reward_dict, agent["log_dir"])
